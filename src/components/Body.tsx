...
import BodySkeleton from "./ui/BodySkeleton";

const PUBLIC_API_IMAGE_PATH = process.env["NEXT_PUBLIC_API_IMAGE_PATH"];

export default function Body() {
  const { data: trendingMovies, isLoading } = UseTrendingMovies("week");
        ...
        <div className={cn("mt-5 grid grid-cols-3 justify-items-center gap-4")}>
          {topTen?.map((movie, index) => (
            ...
            >
              ...
              <div
                className={cn("flex h-[100%] flex-col justify-between py-5")}
              >
